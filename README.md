...
## 📝 Project Overview
This repository contains a production-ready, end-to-end data pipeline that automates data ingestion, cleaning, data quality validation, and visualization of the most streamed tracks on Spotify in 2025. 

The core goal of this project was to move away from rigid, configuration-based enterprise ETL frameworks and build a robust, scalable programmatic pipeline using modern data engineering best practices.

## 📐 Data Architecture & Pipeline Stages
The entire pipeline is fully orchestrated by **Apache Airflow** using the modern **TaskFlow API** and runs within a multi-container **Docker** environment.

`Source CSV (Kaggle) ➔ Apache Airflow ➔ Data Quality Check ➔ PostgreSQL DWH ➔ Apache Superset`

1. **Extract**: Programmatically ingests raw data from the Spotify 2025 CSV dataset using safe system paths via Python's `os` and `csv` modules.
2. **Transform**: Validates data types, cleans strings, handles potential missing values, and calculates custom business metrics (e.g., dynamically flags `is_mega_hit` tracks based on streaming thresholds) without mutating initial data structures.
...
4. **Load (DWH)**: Executes high-performance `UPSERT` operations (`INSERT ... ON CONFLICT DO UPDATE`) into a PostgreSQL database, completely eliminating data duplication and ensuring strict data warehouse consistency.
...
* **Automated Retries**: Configured globally via `default_args` using a 3-tier retry policy with `timedelta` intervals to handle potential database drops or resource deadlocks gracefully.
